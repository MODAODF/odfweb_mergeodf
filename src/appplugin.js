...
(function(OCA) {
	 ...
	 *
	 * @namespace OCA.Files.MergeODFAPPPlugin
	*/
	OCA.Files.MergeODFAPPPlugin = {
		...
		attach: function () {
			var self = this;
			$('#app-content-applist').on('show.plugin-mergeodf', function (e) {
				self.showFileList($(e.target));
			});
			$('#app-content-applist').on('hide.plugin-mergeodf', function () {
				self.hideFileList();
			});
		},
		...
		detach: function () {
			if (this.mergeodfFileList) {
				...
				$('#app-content-applist').off('.plugin-mergeodf');
		...
		_createMergeODFAPPFileList: function ($el) {
			...
			return new OCA.Files.MergeODFAPPFileList(
				$el, {
...
OC.Plugins.register('OCA.Files.App', OCA.Files.MergeODFAPPPlugin);
