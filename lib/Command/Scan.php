...
class Scan extends FolderCommand {
	...
	protected function configure() {
		$this
			->setName('mergeodf:scan')
			->setDescription('Scan a mergeodf for outside changes')
	...
	protected function execute(InputInterface $input, OutputInterface $output) {
		...
		if ($scanner instanceof NoopScanner) {
			$output->writeln("Scanning mergeodf using an object store as primary storage is not supported.");
			return -1;
		}
