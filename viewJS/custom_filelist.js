	...
	_.defer(function() {
		if (window.location.search.split('&').length >= 2) {
			...
			if ($('#app-navigation').find('li').length !== 0) {
				const view_id = $($('#app-navigation').find('li')[0]).attr('data-id')
...
$(document).ready(function() {
	(function(OCA) {
		...
		$('#app-navigation >> li').each((index, content) => {
			const data_id = $(content).attr('data-id')
		...
		for (let i = 0; i < app_id.length; i++) {
			const list_id = app_id[i]['data-id'].split('-')[1]

			const MergeODFAPPFileList = function($el, options) {
			...
			MergeODFAPPFileList.prototype = _.extend({}, OCA.Files.FileList.prototype,
				/** @lends OCA.Files.MergeODFAPPFileList.prototype */ {
					...
					reloadCallback(result) {
						...
						this.hideMask()
						const callBack = this.updateDirInfoCallback.bind(this)
						$.ajax({
					...
					_updateDirectoryPermissions() {
						const isCreatable = (this.dirInfo.permissions & OC.PERMISSION_CREATE) !== 0 && this.$el.find('#free_space').val() !== '0'
						this.$el.find('#permissions').val(this.dirInfo.permissions)
						this.$el.find('.creatable').toggleClass('hidden', !isCreatable)
						this.$el.find('.notCreatable').toggleClass('hidden', isCreatable)
					},
					_onClickNewButton(event) {
